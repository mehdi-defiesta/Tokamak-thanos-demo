...
import {IOptimismMintableERC20} from "contracts/universal/IOptimismMintableERC20.sol";
import {Predeploys} from "contracts/libraries/Predeploys.sol";
import {StandardBridge} from "contracts/universal/StandardBridge.sol";
...
contract L1StandardBridge is StandardBridge, OnApprove, ISemver {
    ...
    function initialize(CrossDomainMessenger _messenger, SystemConfig _systemConfig, SuperchainConfig _superchainConfig)
        public
    ...
    /// @param _data     Data used in OnApprove contract
    function onApprove(address _owner, address, uint256 _amount, bytes calldata _data)
        external
        override
        returns (bool)
    {
    ...
    ///                     Data supplied here will not be used to execute any code on L2 and is
    ///                     only emitted as extra data for the convenience of off-chain tooling.
    function depositNativeTokenTo(address _to, uint256 _amount, uint32 _minGasLimit, bytes calldata _extraData)
        external
    {
    ...
    ///                     Data supplied here will not be used to execute any code on L2 and is
    ///                     only emitted as extra data for the convenience of off-chain tooling.
    function bridgeNativeTokenTo(address _to, uint256 _amount, uint32 _minGasLimit, bytes calldata _extraData)
        external
    {
        _initiateBridgeNativeToken(msg.sender, _to, _amount, _minGasLimit, _extraData);
    }
